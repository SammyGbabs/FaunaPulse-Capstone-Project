...
const char* SUPABASE_SENSOR_TABLE = "environmental_data";  // New table for sensor data
...
void uploadSensorData() {
  ...
  if (httpResponseCode > 0) {
    Serial.println("📤 Sensor data uploaded successfully!");
    Serial.printf("📊 T:%.1f°C H:%.1f%% M:%d%%\n", currentTemperature, currentHumidity, currentMoisture);
    Serial.println("📝 Response: " + http.getString());  // ADD THIS
  } else {
    Serial.print("❌ Sensor upload failed: ");
    Serial.println(httpResponseCode);
    Serial.println("📝 Error Response: " + http.getString());  // ADD THIS TOO
  }
